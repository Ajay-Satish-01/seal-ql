...
payload = {"title": "Fix for issue #59", "body": "Closes #59\n\nImplemented automated fix.", "head": "KartavyaDikshit:fix-issue-59", "base": "main"}
req = urllib.request.Request("https://api.github.com/repos/Ajay-Satish-01/seal-ql/pulls", data=json.dumps(payload).encode(), headers={'Authorization': f'token {token}', 'Accept': 'application/vnd.github.v3+json', 'Content-Type': 'application/json'}, method='POST')
try:
    with urllib.request.urlopen(req, context=ctx) as r: 
        print("[+] PR Created:", json.loads(r.read())['html_url'])
        os.chdir("..")
        shutil.rmtree("/Users/kartavyadikshit/Projects/Open Source/seal-ql_bounty_parallel_18", ignore_errors=True)
