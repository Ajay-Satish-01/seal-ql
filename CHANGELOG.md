...
Drivers are first-class dependencies of `seal-core` and `seal-sql` (`aiomysql`, `aiosqlite`, `clickhouse-connect`), the same pattern as `asyncpg` and `duckdb`. LLM-generated SQL still passes through the SQLGlot zero-trust validator and sanitizer for every dialect; ClickHouse mutations and DDL remain blocked.
