...
import type { CatalogMatchItem, ChatMetadata, ExecutionMetadata } from '@seal/metadata-contract';
...
function tabHasContent(tab: TrustTab, input: TrustSurfaceInput): boolean {
  ...
  switch (tab) {
    ...
    case 'provenance':
      return Boolean(
        meta?.tables_used?.length ||
          meta?.columns_used?.length ||
          meta?.catalog_matches?.length,
      );
    case 'scope':
      return Boolean(
        chatMeta?.scope ||
...
export function TrustPanel({
  ...
  return (
    ...
    >
      ...
      {tabs.length > 0 ? (
        <div
          className="border-border/60 flex flex-wrap gap-1 border-b pb-0.5"
          role="tablist"
      ...
      <div role="tabpanel" className="min-h-[4rem]">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {metadata.tables_used.map((table) => (
                    <span
                      key={table}
                      className="bg-muted/40 rounded-md px-2 py-1 font-mono text-xs"
                    >
                      {table}
                    </span>
                  ))}
              ...
              <div>
                ...
                <ul className="space-y-2">
                  {metadata.catalog_matches.map((match) => (
                    <li
                      key={catalogLabel(match)}
                      className="border-border/40 rounded-md border px-3 py-2"
                    >
        ...
        {activeTab === 'scope' && metadata ? (
          <div className="space-y-3 text-sm">
            ...
            {chatMeta?.refusal ? (
              <p className="text-destructive font-mono text-xs">refusal: true</p>
            ) : null}
            {chatMeta?.suggested_queries && chatMeta.suggested_queries.length > 0 ? (
              <div>
                <p className="text-muted-foreground mb-2 text-[11px] font-medium tracking-wider uppercase">
