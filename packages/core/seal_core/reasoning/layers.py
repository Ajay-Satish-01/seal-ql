...
_VAGUE_QUESTION_MARKERS: tuple[str, ...] = (
    "show me",
    "tell me",
    "what about",
    "how are",
...
def _needs_clarification_heuristic(ctx: ReasoningContext) -> tuple[bool, list[str]]:
    ...
    if len(text) < 8 and not has_specific:
        questions.append("What specific metric or table should I focus on?")
    ...
    has_vague = any(marker in lower for marker in _VAGUE_QUESTION_MARKERS)
    ...
    elif ambiguous_hits >= 3 and not has_specific:
    ...
    async def run(self, ctx: ReasoningContext) -> ReasoningLayerResult:
        try:
            ...
            if len(prior_topics) > 1:
                inferred.append(
                    f"Related prior topics: {'; '.join(t[:60] for t in prior_topics[:-1])}"
                )
            ...
            logger.warning("inferred_context layer failed: %s", exc)
            return ReasoningLayerResult(
                layer_name=self.name,
...
class ClarificationLayer:
    ...
    async def run(self, ctx: ReasoningContext) -> ReasoningLayerResult:
        try:
            ...
            logger.warning("clarification layer failed: %s", exc)
            return ReasoningLayerResult(
                layer_name=self.name,
                unavailable_reason="layer_error",
            )
        ...
        try:
            ...
            if ctx.exec_result and ctx.exec_result.row_count > 0:
                ...
                if ctx.database_capabilities.supports_time_series:
                    followups.append(
                        "Show the same metric over time to spot trends or seasonality."
                    )
            ...
            logger.warning("analysis_followups layer failed: %s", exc)
            return ReasoningLayerResult(
                layer_name=self.name,
                unavailable_reason="layer_error",
            )
        ...
        try:
            ...
            if ctx.exec_result:
                ...
                if ctx.exec_result.truncated:
                    notes.append(
                        "Results were truncated; consider narrowing filters or adding LIMIT."
                    )
            ...
            logger.warning("research_notes layer failed: %s", exc)
            return ReasoningLayerResult(
                layer_name=self.name,
                unavailable_reason="layer_error",
            )
