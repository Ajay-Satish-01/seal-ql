...
```text
       ...
       ┌─────────────────────────────────────────┐
       │            Database Executor            │
       │  Postgres / Timescale / DuckDB / MySQL  │
       │         SQLite / ClickHouse             │
       └────────────────────┬────────────────────┘
                            │
                            ▼
...
* **Layered reasoning (query + chat)**: Optional `metadata.reasoning` — heuristic layers (`REASONING_*` env vars) plus chat `ChatDecision` / answer LLM fields for clarification, follow-ups, and prior-turn context.
...
* **Dialect Intrinsic Execution**: Schema processing and SQL execution for **Postgres (TimescaleDB)**, **DuckDB**, **MySQL/MariaDB**, **SQLite**, and **ClickHouse**. Drivers ship with `seal-core` / `seal-sql` (same as `asyncpg` and `duckdb`): `aiomysql`, `aiosqlite`, and `clickhouse-connect`. Live MySQL/ClickHouse pytest markers (`mysql`, `clickhouse`) need only `SEAL_TEST_MYSQL_URL` / `SEAL_TEST_CLICKHOUSE_URL` — they are skipped in default CI.
