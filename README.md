...
* **Dialect Intrinsic Execution**: Schema processing and SQL execution for **Postgres (TimescaleDB)** and **DuckDB** by default, plus optional **MySQL/MariaDB**, **SQLite**, and **ClickHouse** extras (`aiomysql`, `aiosqlite`, `clickhouse-connect`). Default Compose does not start live MySQL or ClickHouse. Live pytest markers (`mysql`, `clickhouse`) skip unless `SEAL_TEST_*_URL` is set.
