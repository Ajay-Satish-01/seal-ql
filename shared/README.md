...
| `metadata-contract.ts` | Execution metadata types (`ReasoningMetadata`, `ScopeSource`, enhancement reason unions), `STREAM_META_METADATA_KEYS`, `chatResponseToStreamMeta` |
...
HTTP JSON/SSE **wire types** for the TypeScript SDK come from OpenAPI (`make openapi-ts` → `sdks/typescript/src/generated/openapi.ts`), not from this folder. The SDK **vendors** `stream-meta.ts`, `api-error.ts`, `chat-sse-events.ts`, and `metadata-contract.ts` on `prebuild` / `pretest` (`scripts/sync-sdk-meta-vendor.mjs`).
